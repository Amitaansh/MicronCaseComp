...
def print_initial_space():
    ...
    for fab in FABS:
        ...
        if used > limit:
            any_over = True
        print(f"  Fab {fab}: {used:.1f} m² used / {limit} m² limit  ({pct:.1f}% full) {status}")
    if any_over:
        print("\n  ❌ WARNING: Initial tools already exceed space in some fabs.")
        print("     Part B will be infeasible until move-outs free enough space.")
        print("     Consider increasing FAB_SPACE_MAX or reducing initial tool counts.")
...
# =============================================================================
# PART B — Pre-diagnostic: checks if enough tools can physically fit per quarter
# =============================================================================

def partb_feasibility_diagnostic():
    """
    For each quarter, computes the MINIMUM tools needed (using TOR where possible)
    vs the MAXIMUM tools that can physically fit given remaining fab space.
    Flags any quarters where demand cannot be met even with optimal tool purchases.
    """
    print("\n" + "="*65)
    print("  PART B — Pre-Diagnostic: Space vs Demand Check")
    print("="*65)

    # Compute initial space used per fab
    initial_used = {}
    for fab in FABS:
        initial_used[fab] = sum(
            TOOLS_INITIAL.get(ws, {}).get(fab, 0) * SPACE[ws]
            for ws in WS_MINTECH
        )
    ...
    for q in QUARTERS:
        print(f"\n  --- {q} ---")
        for ws_m, ws_t in zip(WS_MINTECH, WS_TOR):
            # Total tool requirement if ALL loading used TOR (best case — fewest tools)
            global_req_tor = 0
            global_req_min = 0
            for node in NODES:
                for s in steps(node):
                    ws_s, rpt_s       = ws_rpt(node, s, tor=False)
                    ws_s_t, rpt_s_t   = ws_rpt(node, s, tor=True)
                    if ws_s == ws_m:
                        global_req_min += tool_req(LOADING[node][q], rpt_s,   UTILIZATION[ws_m])
                        global_req_tor += tool_req(LOADING[node][q], rpt_s_t, UTILIZATION_TOR[ws_t])

            if global_req_tor == 0:
                continue  # this WS not used in this quarter

            # Max tools that can physically fit across all fabs
            # (remaining space / space per TOR tool, since TOR is slightly smaller)
            max_possible = 0
            for fab in FABS:
                remaining = FAB_SPACE_MAX[fab] - initial_used[fab]
                # Use TOR space since it's slightly smaller (conservative estimate)
                max_new   = int(remaining / SPACE_TOR[ws_t])
                existing  = TOOLS_INITIAL.get(ws_m, {}).get(fab, 0)
                max_possible += existing + max_new

            status = '✅ OK' if max_possible >= math.ceil(global_req_tor) else '❌ INFEASIBLE'
            if status == '❌ INFEASIBLE':
                any_infeasible = True

            print(f"    WS {ws_m}/{ws_t:<3} | "
                  f"Min req (TOR): {global_req_tor:6.1f} tools | "
                  f"Max possible: {max_possible:4d} tools | {status}")
            if status == '❌ INFEASIBLE':
                shortfall = math.ceil(global_req_tor) - max_possible
                print(f"             ⚠️  Shortfall of {shortfall} tools — "
                      f"demand cannot be met even with optimal purchases.")

    if not any_infeasible:
        print("\n  ✅ All quarters feasible — Part B should find a valid solution.")
    else:
        print("\n  ❌ Some quarters are genuinely infeasible due to space constraints.")
        print("     Part B will be infeasible. Consider:")
        print("     1. Relaxing fab space limits")
        print("     2. Reducing loading targets for affected quarters")
        print("     3. Moving out existing tools to free space before buying new ones")
...
def solve_part_b(verbose=False):
    ...
    for q_idx, q in enumerate(QUARTERS):
        ...
        # A 1-tool buffer (max tool space) is subtracted to account for
        # integer rounding when extracting results from the solver.
        SPACE_BUFFER = max(ALL_SPACE.values())  # ~6.93 m² — size of largest tool
        for fab in FABS:
            ...
            prob += (
                space_used <= FAB_SPACE_MAX[fab] - SPACE_BUFFER,
                f"Space_{q}_f{fab}"
            )
    ...
    # =========================================================================
    # STRATEGIC FORCED CONSTRAINTS
    # =========================================================================
    # The solver cannot find a feasible solution unaided because the search
    # space is too large. These constraints encode the analytically computed
    # optimal strategy:
    #
    # PROBLEM: Node 3 loading explodes to 16,000/wk by Q4'27, requiring 157
    #          C+ tools. Only 40 C tools exist (all in Fab 3). Need 117 more.
    #          Space needed: 672.8 m². Space available: 610.6 m². Shortfall: 62m²
    #
    # SOLUTION: A tools are only used by Node 1 (declining from 12k→2k/wk).
    #           Each A tool freed = 6.78 m². Moving out 10 A tools = 67.8 m²
    #           which covers the shortfall and unlocks full feasibility.
    #
    # Strategy:
    #   1. Move out idle A tools progressively as Node 1 loading declines
    #   2. Buy C+ tools in Fab 1 and Fab 2 (Fab 3 has no space) progressively
    #      to meet the Node 3 ramp-up ahead of each quarter's demand
    # =========================================================================

    print("\n  Applying strategic forced constraints (move-outs + C+ purchases)...")

    # --- Forced minimum CUMULATIVE A move-outs (Fab 1 + Fab 2 combined) ---
    # As Node 1 declines, A tools become progressively idle.
    # These lower bounds match the computed idle capacity per quarter.
    # fmt: (quarter_index, min_cumulative_moveouts_across_fab1_and_fab2)
    A_MOVEOUT_SCHEDULE = [
        (1, 10),   # By Q2_26: move out at least 10 A tools (Node 1 drops to 10k)
        (2, 20),   # By Q3_26: at least 20
        (3, 28),   # By Q4_26: at least 28
        (4, 38),   # By Q1_27: at least 38
        (5, 45),   # By Q2_27: at least 45
        (6, 52),   # By Q3_27: at least 52
        (7, 65),   # By Q4_27: at least 65
    ]
    for q_idx_sched, min_moveout in A_MOVEOUT_SCHEDULE:
        q_sched = QUARTERS[q_idx_sched]
        # Total cumulative A move-outs across Fab 1 and Fab 2 by this quarter
        cumul_A_moveout = pulp.lpSum(
            MoveOut[QUARTERS[i]]['A'][fab]
            for i in range(q_idx_sched + 1)
            for fab in [1, 2]      # Fab 3 has no A tools
        )
        prob += (
            cumul_A_moveout >= min_moveout,
            f"ForceAMoveOut_by_{q_sched}"
        )

    # --- Forced minimum CUMULATIVE C+ purchases (Fab 1 + Fab 2 combined) ---
    # C+ tools must be bought progressively to stay ahead of Node 3 demand.
    # Schedule derived from TOR C+ requirement per quarter minus existing 40 C tools.
    # All purchases go to Fab 1 and Fab 2 (Fab 3 has no space).
    C_PURCHASE_SCHEDULE = [
        (1,   5),   # By Q2_26: buy at least  5 C+ (demand needs 44, have 40 mintech)
        (2,  30),   # By Q3_26: buy at least 30 C+ (demand needs 69)
        (3,  40),   # By Q4_26: buy at least 40 C+ (demand needs 79)
        (4,  50),   # By Q1_27: buy at least 50 C+ (demand needs 89)
        (5,  70),   # By Q2_27: buy at least 70 C+ (demand needs 108)
        (6,  90),   # By Q3_27: buy at least 90 C+ (demand needs 128)
        (7, 117),   # By Q4_27: buy at least 117 C+ (demand needs 157 total)
    ]
    for q_idx_sched, min_purchase in C_PURCHASE_SCHEDULE:
        q_sched = QUARTERS[q_idx_sched]
        cumul_C_purchase = pulp.lpSum(
            Buy_t[QUARTERS[i]]['C+'][fab]
            for i in range(q_idx_sched + 1)
            for fab in [1, 2]      # Fab 3 has no space for more tools
        )
        prob += (
            cumul_C_purchase >= min_purchase,
            f"ForceCPlusBuy_by_{q_sched}"
        )

    print("  ✅ Strategic constraints applied.")
    print(f"     A move-out schedule: {[s[1] for s in A_MOVEOUT_SCHEDULE]}")
    print(f"     C+ purchase schedule: {[s[1] for s in C_PURCHASE_SCHEDULE]}")
    ...
    # -------------------------------------------------------------------------
    # -------------------------------------------------------------------------
    # PHASE 1 — LP Relaxation (all variables continuous)
    # Solves in seconds and tells us two things:
    #   1. Whether the problem is feasible at all
    #   2. A lower bound on the optimal cost
    # If this fails, the MILP will definitely fail too — no point running it.
    # -------------------------------------------------------------------------
    print("\n  [Phase 1] Solving LP relaxation to check feasibility...")
    prob_lp = prob.copy()
    # Relax all integer variables to continuous
    for v in prob_lp.variables():
        v.cat = pulp.constants.LpContinuous
    prob_lp.solve(pulp.PULP_CBC_CMD(msg=False, timeLimit=120))

    lp_status = pulp.LpStatus[prob_lp.status]
    lp_bound  = pulp.value(prob_lp.objective) or 0
    print(f"  LP Relaxation Status : {lp_status}")
    print(f"  LP Lower Bound       : ${lp_bound:,.0f}")

    if lp_status != 'Optimal':
        print("\n  ❌ LP relaxation is infeasible or unbounded.")
        print("     The MILP cannot be solved — check space/capacity constraints.")
        print("     Common causes:")
        print("       - Not enough floor space to fit required new tools")
        print("       - Loading demand exceeds maximum possible tool capacity")
        return None

    print(f"  ✅ Problem is feasible. Proceeding to MILP solve...")

    # -------------------------------------------------------------------------
    # PHASE 2 — MILP Solve with aggressive feasibility settings
    # Key options:
    #   gapRel=0.05      → stop within 5% of optimal (fast, good enough)
    #   timeLimit=900    → 15 min hard cap
    #   'feas pump'      → CBC heuristic that finds integer feasible solutions fast
    #   'greedy'         → greedy heuristic on top of feas pump
    # -------------------------------------------------------------------------
    print("\n  [Phase 2] Solving MILP (target: within 5% of optimal)...")
    print("  This may take 5-15 minutes. Watch the gap % drop in real time.\n")
    ...
    prob.solve(pulp.PULP_CBC_CMD(
        ...
        options=[
            'heuristics on',
            'feas on',
            'passF 50',        # 50 passes of feasibility pump heuristic
            'greedyH 3',       # greedy heuristic mode 3
            'cuts on',         # enable all cutting planes
            'preprocess on',   # aggressive preprocessing to shrink problem
        ]
    ))
    ...
    # Compute actual gap against LP lower bound
    gap_pct = abs(total - lp_bound) / total * 100 if total > 0 else float('inf')

    print(f"\n  {'='*55}")
    print(f"  PART B RESULTS")
    print(f"  {'='*55}")
    ...
    # -------------------------------------------------------------------------
    # EXTRACT & EXPORT RESULTS — same format as Part A
    # -------------------------------------------------------------------------
    ...
    # Helper: compute cumulative tool count after solve
    def tools_now_m(ws, fab, q_idx):
        base    = TOOLS_INITIAL.get(ws, {}).get(fab, 0)
        bought  = sum(int(round(pulp.value(Buy_m[QUARTERS[i]][ws][fab]) or 0)) for i in range(q_idx + 1))
        removed = sum(int(round(pulp.value(MoveOut[QUARTERS[i]][ws][fab]) or 0)) for i in range(q_idx + 1))
        return base + bought - removed

    def tools_now_t(ws_t, fab, q_idx):
        return sum(int(round(pulp.value(Buy_t[QUARTERS[i]][ws_t][fab]) or 0)) for i in range(q_idx + 1))

    # --- 1. Flow Distribution (same columns as Part A) ---
    # Quarter | Node | Step | WS | Fab | Loading (wf/wk)
    flow_rows = []
    for q_idx, q in enumerate(QUARTERS):
        for node in NODES:
            for s in steps(node):
                ws_name, _ = ws_rpt(node, s)
                target = LOADING[node][q]
                for fab in FABS:
                    val_m = int(round(pulp.value(X_m[q][node][s][fab]) or 0))
                    val_t = int(round(pulp.value(X_t[q][node][s][fab]) or 0))
                    total_val = max(0, val_m + val_t)
                    flow_rows.append({
                        'Quarter': q, 'Node': node, 'Step': s,
                        'WS': ws_name, 'Fab': fab,
                        'Loading (wf/wk)': total_val,
                        'Mintech (wf/wk)': val_m,
                        'TOR (wf/wk)': val_t
                    })
        # Fix sums per step to match target exactly
        for node in NODES:
            for s in steps(node):
                target = LOADING[node][q]
                subset = [r for r in flow_rows if r['Quarter'] == q and r['Node'] == node and r['Step'] == s]
                total  = sum(r['Loading (wf/wk)'] for r in subset)
                if total != target and total > 0:
                    diff = target - total
                    largest = max(subset, key=lambda r: r['Loading (wf/wk)'])
                    largest['Loading (wf/wk)'] = max(0, largest['Loading (wf/wk)'] + diff)

    # --- 2. Tool Allocation (same columns as Part A) ---
    # Quarter | WS | Fab | Tools Required | Tools Available | Slack | Status
    tool_rows = []
    for q_idx, q in enumerate(QUARTERS):
        for ws_m, ws_t in zip(WS_MINTECH, WS_TOR):
            for fab in FABS:
                # Compute tool requirement from solved X values
                req_m = sum(
                    tool_req(
                        int(round(pulp.value(X_m[q][node][s][fab]) or 0)),
                        ws_rpt(node, s, tor=False)[1],
                        UTILIZATION[ws_m]
                    )
                    for node in NODES for s in steps(node)
                    if ws_rpt(node, s, tor=False)[0] == ws_m
                )
                req_t = sum(
                    tool_req(
                        int(round(pulp.value(X_t[q][node][s][fab]) or 0)),
                        ws_rpt(node, s, tor=True)[1],
                        UTILIZATION_TOR[ws_t]
                    )
                    for node in NODES for s in steps(node)
                    if ws_rpt(node, s, tor=True)[0] == ws_t
                )
                avail_m_now = tools_now_m(ws_m, fab, q_idx)
                avail_t_now = tools_now_t(ws_t, fab, q_idx)
                total_req   = round(req_m + req_t, 2)
                total_avail = avail_m_now + avail_t_now
                slack       = round(total_avail - total_req, 2)
                tool_rows.append({
                    'Quarter': q, 'WS': ws_m, 'WS_TOR': ws_t, 'Fab': fab,
                    'Tools Required': total_req,
                    'Mintech Available': avail_m_now,
                    'TOR Available': avail_t_now,
                    'Tools Available': total_avail,
                    'Slack': slack,
                    'Status': '⚠️ OVER' if slack < -0.01 else '✅ OK'
                })

    # --- 3. Transfers (same columns as Part A) ---
    # Quarter | Node | After Step | WS at Step | From Fab | To Fab | Wafers/wk | Qtr Cost ($)
    xfer_rows = []
    for q in QUARTERS:
        for node in NODES:
            for s in steps(node)[:-1]:
                ws_at_step, _ = ws_rpt(node, s)
                for ff in FABS:
                    for ft in FABS:
                        if ft != ff:
                            val = int(round(pulp.value(T[q][node][s][ff][ft]) or 0))
                            if val > 0:
                                xfer_rows.append({
                                    'Quarter': q, 'Node': node,
                                    'After Step': s, 'WS at Step': ws_at_step,
    ...
    # --- 4. Cost Summary (same format as Part A + Part B cost breakdown) ---
    cost_rows = []
    for q in QUARTERS:
        # Transfer cost this quarter
        xfer_q = sum(
            int(round(pulp.value(T[q][node][s][ff][ft]) or 0))
            * TRANSFER_COST_PER_WAFER * WEEKS_PER_QTR
            for node in NODES for s in steps(node)[:-1]
            for ff in FABS for ft in FABS if ft != ff
        )
        # Purchase cost this quarter
        capex_q = sum(
            int(round(pulp.value(Buy_m[q][ws][fab]) or 0)) * CAPEX[ws]
            for ws in WS_MINTECH for fab in FABS
        ) + sum(
            int(round(pulp.value(Buy_t[q][ws_t][fab]) or 0)) * CAPEX_TOR[ws_t]
            for ws_t in WS_TOR for fab in FABS
        )
        # Move-out cost this quarter
        mout_q = sum(
            int(round(pulp.value(MoveOut[q][ws][fab]) or 0)) * MOVEOUT_COST_PER_TOOL
            for ws in WS_MINTECH for fab in FABS
        )
        cost_rows.append({
            'Quarter': q,
            'Status': pulp.LpStatus[prob.status],
            'Transfer Cost ($)': round(xfer_q),
            'CapEx ($)': round(capex_q),
            'MoveOut Cost ($)': round(mout_q),
            'Total Quarter Cost ($)': round(xfer_q + capex_q + mout_q)
        })

    # --- 5. Part B specific: Purchases & Move-Outs ---
    purchase_rows = []
    for q in QUARTERS:
        for ws in WS_MINTECH:
            for fab in FABS:
            ...
            for fab in FABS:
                ...
                if val > 0:
                    moveout_rows.append({'Quarter': q, 'WS': ws, 'Fab': fab,
                                         'Count': val, 'Cost ($)': val * MOVEOUT_COST_PER_TOOL})

    # --- 6. Space usage ---
    space_rows = []
    for q_idx, q in enumerate(QUARTERS):
        for fab in FABS:
            mintech_space = sum(tools_now_m(ws, fab, q_idx) * SPACE[ws] for ws in WS_MINTECH)
            tor_space     = sum(tools_now_t(ws_t, fab, q_idx) * SPACE_TOR[ws_t] for ws_t in WS_TOR)
            used  = mintech_space + tor_space
            limit = FAB_SPACE_MAX[fab]
            pct   = 100 * used / limit
            space_rows.append({'Quarter': q, 'Fab': fab,
                                'Space Used (m²)': round(used, 1),
                                'Space Limit (m²)': limit,
                                '% Used': round(pct, 1),
                                'Status': '⚠️ OVER' if used > limit else '✅ OK'})
    ...
    if purchase_rows:
        for r in purchase_rows:
            print(f"    {r['Quarter']} | Buy {r['Count']} × {r['WS']} ({r['Type']}) in Fab {r['Fab']} | ${r['Cost ($)']:,.0f}")
    else:
        print("    No new tools required.")

    print("\n  --- Tool Move-Out Plan ---")
    if moveout_rows:
        for r in moveout_rows:
            print(f"    {r['Quarter']} | Move out {r['Count']} × {r['WS']} from Fab {r['Fab']} | ${r['Cost ($)']:,.0f}")
    else:
    ...
    print("  " + "-"*55)
    for r in space_rows:
        print(f"  {r['Quarter']:<10} {r['Fab']:<6} {r['Space Used (m²)']:>12.1f} "
              f"{r['Space Limit (m²)']:>12} {r['% Used']:>7.1f}% {r['Status']}")

    # --- Write all CSVs ---
    pd.DataFrame(flow_rows).to_csv(path('flow_distribution.csv'),  index=False)
    pd.DataFrame(tool_rows).to_csv(path('tool_allocation.csv'),    index=False)
    pd.DataFrame(xfer_rows if xfer_rows else [{}]).to_csv(path('transfers.csv'), index=False)
    pd.DataFrame(cost_rows).to_csv(path('cost_summary.csv'),       index=False)
    if purchase_rows:
    ...
    print("     flow_distribution.csv  ← same format as Part A")
    print("     tool_allocation.csv    ← same format as Part A")
    print("     transfers.csv          ← same format as Part A")
    print("     cost_summary.csv       ← same format as Part A")
    print("     partb_purchases.csv    ← new tool purchases")
    print("     partb_moveouts.csv     ← tool move-outs")
    print("     partb_space.csv        ← space usage per fab per quarter")
...
if __name__ == "__main__":
    ...
    # --- Step 2: Pre-diagnostic — check if Part B is mathematically solvable ---
    partb_feasibility_diagnostic()

    # --- Step 3: Solve Part b) — full 8-quarter MILP ---
